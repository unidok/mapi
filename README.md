> Документация написана для версии 0.9.1

# MAPI
Module API - ядро для модулей, использующихся на сервере майнкрафт [JustMC](https://justmc.io). Оно похоже на ядро Bukkit, только Bukkit загружает плагины, а MAPI загружает модули.

### Последняя версия: `/module load unidok/mapi`
...
8. [Базовые типы аргументов](#Базовые-типы-аргументов)
9. [Кастомные типы аргументов](#Кастомные-типы-аргументов)
10. [Структура типа аргумента](#Структура-типа-аргумента)
11. [Реестр MI](#Реестр-MI)
12. [Игровые переменные](#Игровые-переменные)
13. [Сохранённые переменные](#Сохранённые-переменные)
...
> Необходимые зависимости модуля. Без них модуль будет отключён: он будет подсвечен красным в команде `about`, и в его "проблемы" будет добавлена ошибка. В ключ указывается имя необходимого модуля, а в значение - его версию.
...
> [!TIP]
> [Базовые типы](#Базовые-типы-аргументов)\
> По умолчанию `Word`.
---
### `name`: **Текст**
...
# Базовые типы аргументов
### `Integer`: **Число**
> Целое число.\
> Псевдонимы: `Int`, `int`, `integer`
---
### `Float`: **Число**
> Вещественное число.\
> Псевдоним: `float`
...
---
### `String`: **Текст**
> Строка. (Текст, в котором могут быть пробелы)\
> Псевдонимы: `str`, `string`
---
### `Player`: **Текст**
> Онлайн-игрок. Пользователь может вводить как UUID, так и имя игрока, причём в произвольном регистре. Выдаёт имя игрока.\
> Псевдоним: `player`
---
### `User`: **Текст**
> Оффлайн-игрок. Пользователь может вводить как UUID, так и имя игрока. Если игрок не существует (никогда не был в вашем мире), то выдаст ошибку. Выдаёт имя игрока.\
> Псевдоним: `user`
...
MAPI предоставляет [базовые типы аргументов](#Базовые-типы-аргументов), но также вы можете создать свой тип аргумента.
...
---
### `wordIndex`: **Число**
> Индекс аргумента (элемента) в вводе.
---
### `#arg`: **Текст**
...
```
Есть [основной реестр](https://raw.githubusercontent.com/unidok/mapi/main/mi/main), который загружается по умолчанию, но можно создавать и собственные:
1. Создайте JSON-файл со специальной структурой, указанной выше.
2. Конвертируйте его при помощи [программы](https://github.com/unidok/mapi/blob/main/mi/jsonregistry_converter.py).
...
### `mapi`: **Число**
> Если существует, MAPI загружено в мире.
---
### `modules`: **Список<Текст>**
...
> Задержка напоминания о перезагрузке реестров. (В днях, по умолчанию 14 дней)
---
### `mapiDebug`: **Число**
